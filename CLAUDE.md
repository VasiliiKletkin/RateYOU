...
There are also DB/Redis inspection targets (`psql`, `dbconns`, `dbsizes`, `redis-fsm-keys`, `redis-skip-keys`, `redis-flush`) and `make funnel [D=7]` (acquisition funnel by channel) — read the Makefile before hand-rolling a `docker compose exec`.
...
Registration + routing. On the new-user branch (and only there), several writes land in **one transaction**: the `users` row, a `WELCOME_BONUS_DAYS = 30` premium BONUS grant (the idempotent early-return for existing users is the double-grant guard), and the attribution — either a *pending* `Referral` (when the deep-link payload resolves to a known, non-self user) or an `acquisitions` row for a campaign tag; never both, since both persist into the same one-row-per-user acquisition tables. Malformed/unknown/self payloads are silently dropped; registration always succeeds.

**The `/start` payload carries two kinds of source in one slot**, told apart by shape: numeric is a referral link (`/refer` hands out `?start=<telegram_id>`), anything else is a campaign tag (`?start=habr`). `_extract_referrer_telegram_id` and `_extract_acquisition_source` in `handlers/start.py` split them; numeric-but-unusable values (`0`, `-1`) are broken referral links and become neither. Attribution rides the new-user branch only, so it always means "where they came from", never "where they last came from".

**Sources are unified**: "where a user came from" is one storage for campaign tags and referrers alike. The dictionary `acquisition_sources(id, code, referrer_id NULL)` holds both shapes — a campaign tag (`referrer_id` NULL; auto-created by a race-safe get-or-create on first arrival, or pre-creatable via the admin) and a person (`referrer_id` set, code = their telegram_id; the shapes can't collide on `code` because numeric payloads never reach the tag path). The link `acquisitions(user_id PK, source_id, rewarded_at NULL)` records the one source per user, plus the referral-reward lifecycle in `rewarded_at` (meaningful only for person-sources). **There is no `referrals` table** — migration `a4f1c8e02b57` absorbed it. The domain keeps both aggregates apart: `Acquisition` (marketing fact, deliberately not a column on `User`) and `Referral` (the reward ledger) both persist into these tables — `ReferralRepository` maps a `Referral` to a person-source acquisition, with `ReferralId` = the referee's user UUID (referee-unique by PK). Read the funnel with `make funnel`, which collapses all person-sources into one `(referral)` bucket (a CASE on `referrer_id`) so a thousand individual referrers don't drown the report, joins only 1:1 tables, and uses `EXISTS` for ratings, whose 1:N join would inflate every other column. /start is also where the cached `@username` is (re)captured — on every call, not just the first. Then: search location set → "welcome back" naming /feed and /create; not set → welcome text + browse onboarding (`BrowseOnboarding` FSM): "who would you like to rate?" (`genderpref:` buttons → `SearchPreferences.gender_preference`), then the city picker.
...
### `/setcity` — browse onboarding & search-origin picker (`BrowseOnboarding` FSM)
...
- **`browse_onboarding.py` owns the browse onboarding** (`BrowseOnboarding` FSM: gender preference → city, `/setcity` re-enters at the city step), reused by `/start` and `/feed` when no search area is set. Its router is registered **last** in `handlers/__init__.py` — it has an in-state catch-all that must not shadow other routers' commands — and its text handler filters out `/`-prefixed messages so a command typed mid-flow isn't geocoded as a city name. `feed.py` keeps its own local copy of the entry (`_start_browse_onboarding`) because `browse_onboarding` imports `show_next_or_done` from `feed` — importing back would create a cycle. Renaming the FSM group after a deploy would strand mid-flow users: aiogram stores the active state as a literal `"BrowseOnboarding:<step>"` string in Redis.
