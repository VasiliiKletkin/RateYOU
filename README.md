...
- [ ] **Наполнить базу ~1000 анкет (девушки и парни) из Instagram.** Нужен сид-скрипт: собрать фото/имя/возраст, разложить по `users` + `profiles` (+ `profile_photos`). Решить вопрос с правовой стороной (согласие на использование фото) и пометкой таких анкет, чтобы отличать их от живых пользователей.
